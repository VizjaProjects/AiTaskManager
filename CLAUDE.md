...
dotnet run --project Ordovita.Api             # uruchom API (port 5259 wg launchSettings; 8080 to mapowanie Dockera)
...
- **Hangfire** (maile w tle): dashboard pod `{backend}/hangfire` — **poza** `/api/v1`, więc musi mieć własny `location` w [nginx.conf](client/nginx.conf) (front i backend dzielą origin, a `EXPO_PUBLIC_API_URL` w compose jest **puste** → `getApiBaseUrl()` zwraca origin frontu). W Development [filtr](DotNetServer/Ordovita.Api/Common/HangfireDashboardAuthorizationFilter.cs) wpuszcza bez logowania; w produkcji wymaga `IsInRole("ADMIN")` z `HttpContext.User`, a aplikacja uwierzytelnia się **Bearerem**, nie ciasteczkiem — więc **z przeglądarki na produkcji się nie wejdzie**. Link w SideNavBar (tylko ADMIN, nowa karta).
- **Szablony maili** ([EmailTemplates/](DotNetServer/Ordovita.Api/EmailTemplates/)) trzymają język wizualny Areny, ale **hexem wprost** — w mailu nie ma tokenów CSS ani `global.css`. Zmieniając motyw aplikacji trzeba je poprawić ręcznie. Placeholdery `{{...}}` muszą się zgadzać **co do znaku** ze słownikami w [SmtpIdentityEmailSender.cs](DotNetServer/Ordovita.Infrastructure/Email/SmtpIdentityEmailSender.cs) i [SendEmailNotificationHandler.cs](DotNetServer/Ordovita.Application/Tasks/Comments/SendEmailNotification/SendEmailNotificationHandler.cs) — renderer robi zwykłe `string.Replace`, więc literówka zostawia w mailu surowe `{{cos}}`. ⚠️ `{{fullName}}` przychodzi z **wiodącą spacją** (`$" {FullName}"`), dlatego w szablonie jest `Cześć{{fullName}}!` bez spacji przed.
...
Zakresowane po workspace: `["tasks", wsId]`, `["events", wsId]`, `["categories", wsId]`, `["taskStatuses", wsId]`, `["notes", wsId]`, `["noteFolders", wsId]`, `["aiProposals", wsId]`, `["workspaceUsers", wsId]`.
...
| `/workspace` | `POST create`, `GET all`, `GET {workspaceId}`, `GET allWorkspaceUsers/{workspaceId}`, `POST {workspaceId}/assignUsers`, `POST {workspaceId}/assignUsersByEmail`, `PATCH {workspaceId}/removeUsers`, `PUT {workspaceId}/visibility`, `DELETE delete/{workspaceId}` |
...
25. **Powiadomienie o wzmiance działa przez PRZYPISANIE, bo backend nie zna pojęcia wzmianki.** [AddCommentHandler.cs](DotNetServer/Ordovita.Application/Tasks/Comments/AddComment/AddCommentHandler.cs) **nie parsuje `@` z treści** — kolejkuje maila (Hangfire) dla **każdego `task.AssignedUserIds` poza autorem**. Dlatego `TaskCommentsSection` przed `useAddComment` robi `useSetTaskAssignees` z sumą `dotychczasowi ∪ wzmiankowani`; kolejność jest krytyczna, bo handler czyta listę przypisanych **w momencie zapisu komentarza**. Skutki uboczne, o których trzeba wiedzieć: wzmianka **trwale dopisuje** kogoś do zadania (dlatego composer to zapowiada tekstem), a mail dostają **wszyscy** przypisani, nie tylko oznaczony. `PUT task/{id}/assignees` **podmienia całą listę** (`WorkTask.SetAssignees`), więc nigdy nie wysyłaj samych nowych id. Autor jest zawsze wykluczony — **nie da się powiadomić samego siebie**, co myli przy testach z jednego konta. Wzmianka jest zapisywana jako **czysty tekst** `@Imię Nazwisko` (nie token/UUID), bo szablon maila wstawia `commentContent` przez `WebUtility.HtmlEncode` — każdy znacznik byłby widoczny surowy u odbiorcy. Parsowanie: [mentions.ts](client/lib/utils/mentions.ts) (bez polskich ogonków, bo Hermes nie gwarantuje `String.normalize`, a `ł` i tak nie ma dekompozycji NFD). `extractMentionedMembers` i podświetlanie idą **tą samą** ścieżką (`splitMentionText`), żeby to, co widać, nie mogło się rozjechać z tym, kogo przypisujemy.

27. **Panel podpowiedzi `@` zamyka się przez regułę na zapytaniu, nie przez osobny stan.** `findMentionQuery` ([mentions.ts](client/lib/utils/mentions.ts)) otwiera listę tylko gdy: (a) przed `@` jest biały znak albo początek tekstu, (b) zaraz **za kursorem** jest biały znak albo koniec tekstu (żeby nie wyskakiwała w środku tekstu), (c) zapytanie po `@` **nie zawiera żadnego `\s`**. Punkt (c) robi podwójną robotę — obsługuje „spacja/enter chowa listę" i **sam gasi panel po wyborze**, bo wstawiona wzmianka `@Imię Nazwisko ` zawiera spacje. ⚠️ Nie „naprawiaj" tego dokładaniem flagi `justPicked`: bez reguły (c) po wstawieniu `@Jan Kowalski ` zapytanie `Jan Kowalski ` nadal pasuje do „Jan Kowalski" (bo matcher robi `trim()`) i panel zostaje otwarty — to był pierwotny bug. Konsekwencja projektowa: **nazwiska nie da się dopisać ręcznie po spacji**, trzeba wybrać z listy — i o to chodzi, bo tylko wtedy tekst zgadza się co do znaku z `fullName`, po którym podświetlanie szuka wzmianek.

26. **`GET /workspace/allWorkspaceUsers/{id}` połyka błędy i nie sprawdza dostępu.** [UserWorkspace.cs](DotNetServer/Ordovita.Infrastructure/Workspace/UserWorkspace.cs) opakowuje surowy SQL w `try { … } catch { return []; }`, więc **awaria zapytania jest nieodróżnialna od pustego workspace'u** — brak logu, HTTP 200 z `[]`. Dlatego `useWorkspaceUsers()` ([hooks](client/lib/hooks/index.ts)) przy pustej odpowiedzi schodzi do `activeWorkspace.assignedUsers` ze store'u. Osobno: handler resolvuje wołającego, ale **nie waliduje, czy należy on do workspace'u** — dowolny zalogowany user może odpytać obce `workspaceId`. Endpoint pomija wołającego (`DU.Id != {callingUserIdGuid}`), więc nigdy nie zobaczysz siebie na liście podpowiedzi.

28. **Modal zadania ma DWA rozłączne widoki zakładki „Szczegóły".** Read = `detailsPane` (treść + `propertyRail` po prawej), edit = stary formularz dwukolumnowy. Warunek to `activeTab === "details" && !editing`, więc gałąź `{activeTab === "details" ? …}` wewnątrz `ScrollView` jest osiągalna **tylko w trybie edycji** — nie dokładaj tam elementów widoku read, bo nigdy się nie pokażą. Konsekwencje: komentarze znikają na czas edycji (celowo), a `propertyRail` renderuje się **dwa razy w tym samym drzewie** — jako kolumna 288 px na `isWide`, a poniżej opisu jako karta na wąskim ekranie. Stan `railPicker` jest wspólny dla obu, więc nie da się otworzyć dwóch list naraz (i dobrze).

29. **Edycje z property rail idą przez `applyTaskPatch`, nie przez `editTask.mutate` wprost.** `PUT task` podmienia cały rekord, więc każdy nietknięty field trzeba odesłać z powrotem; `applyTaskPatch` robi to raz i dokłada `resolveTaskDueDateTimeForSave` (pułapka 10). Wołanie `editTask.mutate` z samym `{ statusId }` wyzeruje tytuł, opis i termin. `handleMarkComplete` też przez to przechodzi.

30. **Composer komentarza jest przypięty POZA `ScrollView`-em, a wątek w środku.** Dlatego `TaskCommentsSection` jest rozbity na `TaskCommentsThread` (lista + edycja/usuwanie) i `TaskCommentComposer` (draft + wzmianki + `useSetTaskAssignees`). Oba wołają `useTaskComments`/`useWorkspaceUsers` osobno — React Query dedupikuje po kluczu, więc to nie jest podwójny fetch. Nie scalaj ich z powrotem „dla porządku": scalone wracają do jednego drzewa i composer znowu odjeżdża z przewijaniem.
...
| Komentarze + wzmianki `@` | `TaskCommentsThread` + `TaskCommentComposer` w [TaskModals.tsx](client/components/organisms/TaskModals.tsx), [MentionInput.tsx](client/components/molecules/MentionInput.tsx), [MentionText.tsx](client/components/atoms/MentionText.tsx), [mentions.ts](client/lib/utils/mentions.ts) | `Application/Tasks/Comments/*` (w tym `SendEmailNotification`), `Infrastructure/Jobs/CommentMentionEmailJob.cs` |
...
- **2026-08-13** — Modal zadania przebudowany na **property rail** (projekt „Task detail — property rail"). Powód: widok read tracił większość wysokości na pustą kolumnę metadanych, a komentarze były doklejone pod spodem i odjeżdżały ze scrollem. Teraz: nagłówek z okruszkiem `Zadania › Kategoria` + tytuł z pigułką statusu, liczniki na zakładkach (Kroki `zrobione/wszystkie`, Powiązania `n`), treść po lewej (opis zwijany do 3 linii + „Pokaż pełny opis", wątek komentarzy) i gęsty rail 288 px po prawej (status/priorytet/kategoria • termin/czas/utworzono • przypisani). Status, priorytet i przypisania edytuje się **wprost z raila** rozwijaną listą inline (bez portali — działa też na native). Stopka wg projektu: „Oznacz jako zakończone" + „Edytuj" po lewej, usuwanie po prawej. Tryb edycji zostaje starym formularzem. Frontend-only, zero zmian w API. Nowe klucze i18n `taskModal.showFullDescription`/`showLess` (690 × 2, parytet OK). Pułapki 28–30.
- **2026-08-13** — Maile przebudowane na język wizualny Areny (kremowe `#f5f3ef`, płaska karta z 1px `#e2dfd9`, akcent `#5b4ee0`, Playfair/Georgia na nagłówek, Inter/system na treść) + wariant dark zmapowany na `.dark` z `global.css`. Dotyczy wszystkich trzech szablonów; wcześniej były w niepowiązanej palecie teal+slate. Placeholdery bez zmian. Fix linku do Hangfire: `location /hangfire` w [nginx.conf](client/nginx.conf) — przycisk prowadził na origin frontu (`localhost:3000`), bo nginx proxował tylko `/api/v1/`, `/oauth2/` i `/login/oauth2/`.
- **2026-08-13** — Powiadomienie o wzmiance + wejście w Hangfire z panelu admina. Przyczyna „zero jobów w Hangfire": `AddCommentHandler` iteruje po `AssignedUserIds`, a testowe zadanie nie miało przypisanych osób poza autorem, więc pętla nie wykonała się ani razu — nic nie było zepsute, mail po prostu nigdy nie miał adresata. Naprawione **bez ruszania backendu**: `TaskCommentsSection` przed wysłaniem komentarza dopisuje wzmiankowanych do przypisanych (`useSetTaskAssignees` z sumą starych i nowych, bo `PUT assignees` podmienia całą listę), composer zapowiada to tekstem. Nowe `extractMentionedMembers` w [mentions.ts](client/lib/utils/mentions.ts) + granice dopasowania (odstęp przed `@`, brak litery po nazwisku), żeby `@Jan Kowalskiego` nie przypisywało Jana Kowalskiego, a `@Jan Kowalski` nie zaliczało „Jan Kowal". SideNavBar dostał pozycję „Zadania w tle" (tylko ADMIN → `{backend}/hangfire`, nowa karta). Szczegóły i pułapki w 25. Sprostowany port w sekcji Komend: `dotnet run` to **5259**, nie 8080.
- **2026-08-13** — Wzmianki `@` w dyskusji zadania. Po wpisaniu `@` w komentarzu (nowym i edytowanym) pojawia się lista **wszystkich osób z workspace'u** (`GET allWorkspaceUsers`, bez Ciebie); wybór wstawia czysty tekst `@Imię Nazwisko`, a zapisane wzmianki są podświetlane w wątku (wzmianka Ciebie dostaje tło akcentu). Nowe: [mentions.ts](client/lib/utils/mentions.ts), [MentionInput.tsx](client/components/molecules/MentionInput.tsx), [MentionText.tsx](client/components/atoms/MentionText.tsx), typ `WorkspaceMember`, `workspaceApi.getUsers`, hook `useWorkspaceUsers`, klucz `["workspaceUsers", wsId]`, i18n `comments.mention*` (686 kluczy × 2 języki, parytet OK). Backend: **jedyna zmiana to `Guid UserId` w rekordzie `UserWorkspace`** + `DU.Id AS UserId` w SQL — bez tego nie da się przypiąć wzmianki do konkretnego użytkownika. Logika maili świadomie **bez zmian** — leci do przypisanych do zadania, nie do wzmiankowanych, więc `@` i powiadomienie to dwie niezależne rzeczy; szczegóły w pułapkach 25–26. Nawigacja klawiaturą (↑/↓/Enter/Tab/Esc) przez natywny listener `keydown`, bo RN-Web nie gwarantuje strzałek w `onKeyPress`. Warunki otwarcia panelu (odstęp przed `@`, koniec słowa za kursorem, brak spacji w zapytaniu) — pułapka 27.
- **2026-08-11** — Kalendarz: wybór koloru **przy tworzeniu** wydarzenia. Kolor nie istniał w tej ścieżce w żadnej warstwie — `CalendarEvent.Create` wpisywał `DefaultColor` na sztywno i nie przyjmował parametru. Pole przeprowadzone przez domenę (opcjonalny `string? color`, fallback zostaje w domenie), `CreateCalendarEventCommand`, `CreateEventRequest` (API i front) do `CreateEventModal`. Pozostałe 6 wywołań `CalendarEvent.Create` (auto-event z terminu zadania, akceptacja propozycji AI, planowanie LLM) bez zmian. Paleta próbek wyciągnięta do `EventColorPicker` współdzielonego przez oba modale; pierścień zaznaczenia był `#111827` (generyczna szarość Tailwinda, niewidoczna w dark mode) → token zależny od motywu. Modal tworzenia dostał `ScrollView` + `max-h-[90%]` (pułapka 20). ⚠️ Backendowy `CalendarEvent.DefaultColor` (`#4d41df`) nadal różni się od frontowego `DEFAULT_EVENT_COLOR` (`#5b4ee0`) i nie ma go w palecie — dotyczy tylko wydarzeń tworzonych automatycznie.
- **2026-08-11** — [WorkspaceModal.tsx](client/components/organisms/WorkspaceModal.tsx): naprawiony ciemny motyw. Komponent w ogóle nie czytał `useThemeStore`, więc kolory ikon były zaszyte pod jasny tryb. Ikona aktywnego workspace'u (`#ffffff` na `bg-inverse-surface`, które w dark mode jest kremowe) znikała, dając pusty biały kwadrat; ptaszek `#111111` i „+" w „Create workspace" też były niewidoczne; akcent nie przełączał się na `#9b8cff`. Helper `iconColors(isDark)` mapuje tokeny z `global.css` na inline'owy prop `color` MaterialIcons. Jasny tryb bez zmian wizualnych.
