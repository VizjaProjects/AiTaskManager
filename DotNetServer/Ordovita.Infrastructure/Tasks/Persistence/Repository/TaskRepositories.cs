...
{
    public async Task<TaskCategory> AddAsync(TaskCategory category, CancellationToken ct = default)
    {
    ...
    {
        return await context.TaskCategories.FirstOrDefaultAsync(c => c.Id == id, ct);
    }

    public async Task<int> CountByWorkspaceIdAsync(WorkspaceId workspaceId, CancellationToken ct = default)
    {
        return await context.TaskCategories.CountAsync(c => c.WorkspaceId == workspaceId, ct);
    }

    public async Task<IReadOnlyList<TaskCategory>> GetByWorkspaceIdAsync(
        WorkspaceId workspaceId, CancellationToken ct = default)
    {
        return await context.TaskCategories.AsNoTracking()
...
public sealed class WorkTaskStatusRepository(AppDbContext context) : IWorkTaskStatusRepository
{
    ...
    {
        return await context.WorkTaskStatuses.FirstOrDefaultAsync(s => s.Id == id, ct);
    }

    public async Task<IReadOnlyList<WorkTaskStatus>> GetByWorkspaceIdAsync(
        WorkspaceId workspaceId, CancellationToken ct = default)
    {
        return await context.WorkTaskStatuses.AsNoTracking()
            .Where(s => s.WorkspaceId == workspaceId)
            .ToListAsync(ct);
    }

    public void Delete(WorkTaskStatus status)
    {
        context.WorkTaskStatuses.Remove(status);
    }
}

public sealed class WorkCalendarRepository(AppDbContext context) : IWorkCalendarRepository
{
    public async Task<WorkCalendar?> GetByUserIdAsync(WorkspaceId workspaceId, CancellationToken ct = default)
    {
        return await context.WorkCalendars.FirstOrDefaultAsync(c => c.WorkspaceId == workspaceId, ct);
    }
    ...
    public async Task<WorkCalendar?> GetPrimaryByWorkspaceIdAsync(
        WorkspaceId workspaceId, CancellationToken ct = default)
    {
        return await context.WorkCalendars.FirstOrDefaultAsync(c => c.WorkspaceId == workspaceId && c.IsPrimary, ct);
    }
}
