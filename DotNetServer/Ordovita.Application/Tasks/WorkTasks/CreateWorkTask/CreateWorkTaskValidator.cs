...
public sealed class CreateWorkTaskValidator : AbstractValidator<CreateWorkTaskCommand>
{
    public CreateWorkTaskValidator()
    {
        ...
        RuleFor(x => x.Steps).NotNull().Must(steps => steps.Count <= Domain.Tasks.WorkTask.MaxSteps)
            .WithMessage("A task cannot have more than 20 steps.");
        RuleForEach(x => x.Steps).ChildRules(step =>
        {
            step.RuleFor(value => value.Title).NotEmpty().MaximumLength(Domain.Tasks.WorkTask.StepTitleMaxLength);
